        ...
        <div class="row">
          <div class="col-lg-12 d-flex "  >
              ...
              <div class="col-lg-2" style="width: 100%;">
                <a href="{{ route('admin-publikasi.create') }}" class="btn btn-purple bg-purple"> <i class="fa fa-plus"></i> Buat @yield('title')</a>
              </div>
              ...
              <div class="card-body" id="card-table" style="overflow-x: auto; overflow-y: auto; max-height: 400px;">
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <tbody>
                      @forelse($publikasi as $kg)
                        <tr id="tampil-menu">
                            @if(empty($kg->file) || is_null($kg->file))
                                <td>No File Available</td>
                            @else
                                <td>
                                    <!-- Link to download the PDF file -->
                                    <a href="{{ Storage::url('file-publikasi/' . $kg->file) }}" target="_blank">
                                        Download PDF / lihat file
                                    </a>
                                </td>
                            @endif
                            <td>{!! $kg->name !!}</td>
                            ...
                            <td> 
                                <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('admin-publikasi.destroy', $kg->id) }}" method="POST">
                                    <a href="{{ route('admin-publikasi.edit', $kg->id) }}" class="btn btn-warning btn-sm text-white"><i class="fa fa-edit"></i></a> 
